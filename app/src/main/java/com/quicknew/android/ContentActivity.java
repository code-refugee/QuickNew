...
//        refresh.setColorSchemeColors(Color.parseColor("#8000"));
    ...
    private String analyseShare(){
        return "想让自己的思路跟上市场状态、踩准节奏，最后获得成功，最聪明" +
                "的办法就是深入研究同行业的股票，从而找到好的板块，同时辨认出坏" +
                "的板块。如果发现有的板块很有希望上涨，那么赶紧买入；但是如果有" +
                "板块行情转坏，那么赶紧退出。哪些疲软的板块碰都不要碰。交易员必须" +
                "随时准备改变自己之前的预测，有能力根据行情发展调整自己的头寸。如果" +
                "强势板块中的某一支股票没有随同其他行业股票一起上场，这至少说明这支" +
                "股票比较虚，或者说病了，因此是一个做空的良机";
    }
